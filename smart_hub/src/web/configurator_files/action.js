...
function initActElements(act_code, act_args) {
  if (switching_act.has(act_code)) {
    ...
    if (act_code < 4) {
      ...
      else setElement("timeunit-act", "1");
    }
    if (out_no < 16) {
      ...
      if (cnt_no % 2 == 0) setElement("countopt-act", 2);
      else setElement("countopt-act", 1);
    } else {
      setElement("action-select", 111);
      if (out_no > 100) out_no -= 100;
      else if (out_no < 33) out_no -= 24;
      setElement("flag-act", out_no);
    }
    if (act_code == 9) {
      if (act_args[2] == 255) setElement("outopt-act", 6);
    ...
    } else setElement("outopt-act", act_code);
    ...
    if (act_args[2] == 100) {
      if (act_args[0] == 2) setElement("rgb-opts", 2);
      else {
        if (sel_col == amb_white.toString()) setElement("rgb-opts", 11);
        else if (sel_col == amb_warm.toString()) setElement("rgb-opts", 12);
        else if (sel_col == amb_cool.toString()) setElement("rgb-opts", 13);
        else {
      ...
      if (act_args[0] == 2) setElement("rgb-opts", 2);
        ...
        if (sel_col == col_red.toString()) setElement("rgb-opts", 5);
        else if (sel_col == col_green.toString()) setElement("rgb-opts", 6);
        else if (sel_col == col_blue.toString()) setElement("rgb-opts", 7);
        else if (sel_col == col_white.toString()) setElement("rgb-opts", 10);
        else {
          setElement("rgb-opts", 4);
          col_str = colArray2StrConv([act_args[3], act_args[4], act_args[5]]);
          document.getElementById("rgb-colorpicker").value = col_str;
    ...
    if (act_code > 220) {
      ...
      if ((cl_md > 10) & (cl_md < 15)) setElement("climopt-act", cl_md);
      else if (cl_md > 20) {
        cl_md -= 20;
  ...
  if (idx > 3) {
    ...
    if (act_sel.value == "act-111") {
      ...
      for (var i = 0; i < flg_sel.length; i++) {
        ...
        if ((flg_idx > 8) & (flg_idx < 17)) flg_sel.options[i].disabled = true;
        else if (flg_idx > 40) flg_sel.options[i].disabled = true;
      }
      flg_idx = flg_sel[flg_sel.selectedIndex].value.split("-")[1];
      if ((flg_idx > 8) & (flg_idx < 17)) flg_sel.selectedIndex = 0;
      else if (flg_idx > 40) flg_sel.selectedIndex = 0;
    }
  } else {
    if (act_sel.value == "act-111") {
      var flg_sel = document.getElementById("flag-act");
...
function disablePercval() {
  ...
  if (cvr_opt > 10) setElementVisibility("perc-val", "hidden");
  else setElementVisibility("perc-val", "visible");
}
...
function setMaxCountAct() {
  ...
  if (cnt_val.value > max_cnt_val) cnt_val.value = max_cnt_val;
}
...
function setRGBColorOptions() {
  ...
  if (col_sel.options[col_sel.selectedIndex].disabled) {
    if (col_sel.value == 5) col_str = colArray2StrConv(col_red);
    else if (col_sel.value == 6) col_str = colArray2StrConv(col_green);
    else if (col_sel.value == 7) col_str = colArray2StrConv(col_blue);
    else if (col_sel.value == 10) col_str = colArray2StrConv(col_white);
    else if (col_sel.value == 11) col_str = colArray2StrConv(amb_white);
    else if (col_sel.value == 12) col_str = colArray2StrConv(amb_warm);
    else if (col_sel.value == 13) col_str = colArray2StrConv(amb_cool);
    col_sel.value = 4;
...
function setRGBPicker() {
  ...
  else setElementVisibility("rgb-colorpicker", "hidden");
}
...
function setMsgTime() {
  ...
  else setElementVisibility("msgset-time", "hidden");
}
