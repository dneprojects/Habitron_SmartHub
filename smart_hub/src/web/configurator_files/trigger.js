...
const sensor_trg = new Set([
  201, 202, 203, 204, 205, 213, 214, 215, 216, 217, 218, 219,
]);
...
function initTrigElements(trg_code, trg_arg1, trg_arg2, trg_time) {
  if (button_trg.has(trg_code)) {
    ...
    if (trg_arg1 > 0) setElement("onoff-select", 1);
    else setElement("onoff-select", 2);
    ...
    if (trg_arg1 > 0) setElement("flag2-select", 1);
    else setElement("flag2-select", 2);
...
function setMaxCount() {
  ...
  for (var i = 0; i < cnt_sel.length; i++) {
    if (i > max_cnt_val) cnt_sel.options[i].disabled = true;
    else cnt_sel.options[i].disabled = false;
  }
  if (cnt_sel.selectedIndex > max_cnt_val) cnt_sel.selectedIndex = 0;
}
...
function setEkeyUsrFingers() {
  ...
  for (var i = 0; i < 10; i++) {
    var mask = 1 << i;
    if (finger_mask & mask) finger_sel.options[i + 1].disabled = false;
    else finger_sel.options[i + 1].disabled = true;
  }
  if (finger_sel.options[finger_sel.selectedIndex].disabled)
    finger_sel.selectedIndex = 0;
  if (usr_parts[0] == 255) finger_sel.style.visibility = "hidden";
  else finger_sel.style.visibility = "visible";
}
